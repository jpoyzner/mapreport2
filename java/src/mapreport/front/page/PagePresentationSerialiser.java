...
public class PagePresentationSerialiser implements JsonSerializer<PagePresentation> {
	...
	public JsonElement serialize(PagePresentation page, final Type typeOfSrc, final JsonSerializationContext context) {
	 ...
	 //   jsonObject.add("view", jsonView);   
	    ...
	    final JsonElement jsonSQL = context.serialize(page.getPst().toString().replaceAll("\u003d", "=").replaceAll("\n", "").replaceAll("\r", "").replaceAll("\t", ""));
	    jsonObject.add("SQL", jsonSQL);
	    
	    final JsonElement jsonView = context.serialize(page.getView()); //.getNewsList());
	    jsonObject.add("view", jsonView);
	    
	    final JsonElement jsonNews = context.serialize(page.getView().getNewsList().getNewses()); //.getNewsList());
